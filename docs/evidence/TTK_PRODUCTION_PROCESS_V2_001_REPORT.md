...
- Final full governance regression after third independent-review remediation `node --test scripts/hooks/hooks.test.mjs` — PASS `111/111`, `0` failures, exit `0`.
...
The regression suite records fail-closed behavior for incomplete or omitted required `product_gate` contracts, canonical/legacy physical-Human gate signal omission (including closed `human_gate_mode` validation), omitted mandatory preflight evidence, non-representative artifacts, producer/source-SHA laundering, scalar-only unsupported readiness, missing **or extra** structured dimensions, missing placeholder-audit evidence, declared confounding/unsupported placeholder dispositions, missing physical-device measurements, committed and dirty player-runtime drift after artifact source SHA, and artifact/build-log SHA-256 mismatch.
...
A second fresh independent read-only review of exact candidate `5e2dd365c8acac92f972ff2767740e302458625a` again returned `FAIL`; no receipt or terminal closeout was authorized or persisted. B1 and B2 retested PASS, while B3 exposed two narrower bypasses: a declared `CONFOUNDING_ACTIVE_QUESTION` placeholder could still pass, and structured evidence could add undeclared extra dimensions. Both were reproduced as RED before repair. The guard now requires exact-set equality between the active `product_gate.representative_dimensions` and every structured dimension set, and placeholder entries may pass only as `REPLACED` or explicitly `ACCEPTED_NON_CONFOUNDING`; confounding/unknown/unsupported dispositions block handoff. The latest Product Gate/preflight focused regression is PASS `17/17`, including both second-review attacks.

A third fresh independent read-only review of exact candidate `0ee0c1e3d943c49b419f1fb69c4e6d9be794cad3` returned `FAIL`; again no receipt or terminal closeout was authorized or persisted. B2 and the B3 adversarial matrix retested PASS, but B1 remained reachable because free-form regex inference missed `PHYSICAL_HUMAN_PRODUCT_ACCEPTANCE_REQUIRED`, `HUMAN_PLAYTEST_REQUIRED`, and `required_evidence.human_product_acceptance`. All three bypasses plus omission under the new canonical mode were reproduced as four RED failures before repair. `pre-task` now uses the closed `human_gate_mode` enum (`NONE` or `PHYSICAL_PRODUCT_ACCEPTANCE`) as the canonical signal for newly authored/updated authority, rejects unknown modes and `NONE`/required-gate contradictions, and uses only a bounded exact legacy-alias registry to fail closed on already-used physical-Human stop/evidence names. No free-form Human/physical regex inference remains. The focused Product Gate contract regression is PASS `12/12`; generic Human successor/merge decisions remain non-product signals.
