...
This report is intentionally not yet contract-complete until the report file itself is included in the final writer-scope recheck.
...
This is the outer history-rewrite boundary paired with repository-local exact-content validation. No force-push probe was attempted because a failed platform rule is sufficient evidence and an active destructive probe would be unsafe if platform behavior were misconfigured.

## P1-2 — Singular aggregate evidence contract

`NEXT_TASK.evidence_file` points only to this report. The regression suite proves that one evidence file containing all declared keys passes and that missing/wrong keys continue to fail closed.

This report will be promoted to final `verdict = PASS` only after a scope compare including this file confirms that every writer path remains within the activated allowlist.
...
## Writer scope before evidence report

Compare `af18d87c... → 91b05ba1...` was `ahead_by=1`, `behind_by=0`, and changed exactly five authorized writer paths:
...
No control-plane, gameplay, Unity, package, project-settings or build path was changed by the writer payload.
...
Final Foreman must now recheck writer scope including this evidence file. If and only if that passes, update this same report to aggregate machine `PASS`, verify CI again, and transition to a fresh independent `REVIEW` state. No merge or successor authority is granted.
