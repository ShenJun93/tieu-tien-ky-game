...
if (!baselineRef || !/^[0-9a-f]{40}$/i.test(baselineRef)) fail('baseline_ref must be an explicit 40-character commit SHA');
let baseline;
try { baseline = run('git', ['rev-parse', '--verify', `${baselineRef}^{commit}`]); }
catch { fail(`baseline commit is missing: ${baselineRef}`); }
if (baseline.toLowerCase() !== baselineRef.toLowerCase()) fail('baseline_ref did not resolve to exact authorized SHA');

const mergeBase = run('git', ['merge-base', 'HEAD', baseline]);
if (mergeBase !== baseline) fail(`branch does not contain authorized baseline ${baseline.slice(0, 12)}`);
